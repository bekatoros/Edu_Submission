...
<table>
					<tr>
						...
						<a href="central.jsp">Σύστημα Ηλ. Υποβολής Ερευνητικού και Εκπαιδευτικού υλικού</a>
						</td>
					</tr>
                            ...
                            if ((session.getAttribute("verified")!=null)&&(Integer)session.getAttribute("verified")==1)
                            {
                                ...
                                if(user.getRole()==1)//administrator
                                {
                                 /*  out.println("<div align='center'><form  action='submit.jsp' method='post' style='width: 211px; height: 27px'>"+
                                    ...
                                    out.println("<table align='center'><tr><td><div align='center'><a class='large blue button' align='center' href='submit.jsp'>Απόθεση νέου Υλικού</a></div></td>");
                                 ...
                                 //    out.println("<div align='center'><a  href='submit.jsp'>Απόθεση νέας Δημοσίευσης</a></div>");                       
                                    //kati tha kanei kai aytos
                                        ...
                                        {  
                                            ...
                                            String sub_id="";
                                            statement = connection.createStatement();
                                            rs = statement.executeQuery("SELECT * FROM ee_Publication WHERE status="+finalstate+" AND user_id="+user.getId()+" ORDER BY RAND() LIMIT 5");                                           
                                            out.println("<h4  align='center'>Υλικό που έχει υποβληθεί</h4>");
                                            ...
                                            out.println("</table>");
                                            out.println("<div align='center' align='center'><a href='active.jsp'  align='center'>Δείτε όλο το υλικό</a></div>");
                                        ...
                                        {  
                                            ...
                                            String sub_id="";
                                            statement = connection.createStatement();
                                            rs = statement.executeQuery("SELECT * FROM ee_Publication WHERE status="+1+" AND user_id="+user.getId()+" ORDER BY RAND() LIMIT 5");                                           
                                            out.println("<h4  align='center'>Υποβολές χωρίς έγγραφο</h4>");
                                            out.println("<table border='1'  align='center'>");
                                            while (rs.next())
                                            {                                 
                                              title=rs.getString("title");  
                                            ...
                                            out.println("</table>");
                                            out.println("<div align='center'><a href='incomplete.jsp'  align='center'>Δείτε όλες τις υποβολές</a></div>");
                                        }
                                        catch(Exception ex)
                                        {
                                            out.println("<p>"+ex.toString()+"</p>") ;
